...
public class Piste {
	...
	private int longueur_segment = Affichage.HAUT/5;
	 ...
	 */
	public Piste() {
		x_current = Affichage.LARG/2;
		y_current = Affichage.HAUT;
		position = 0;
		ligne = new ArrayList<Point>();
		decors = new ArrayList<>();
		prev_line_position = new Point(0,0);     
		prev_line_position_raw = new Point(0,0);
		waitCheck = false;
		isCheckpoint = false; 
	...
	public void avance(Etat etat) {
		...
		if (this.isCheckpoint) {
			this.posYCheckpoint += etat.vaisseau.vitesse/10;
		}
	...
	public void crossCheckpoint(Etat etat) {
		int posBaseAx = positionPiste(posYCheckpoint) - taillePiste(posYCheckpoint)*3/4;
		int posBaseBx = positionPiste(posYCheckpoint) + taillePiste(posYCheckpoint)*3/4;
		if (posYCheckpoint >= (Affichage.SOL-Affichage.HAUT/10) && etat.vaisseau.y >= (Affichage.HAUT*2/3) && (etat.vaisseau.x + Vaisseau.LARG > posBaseAx && etat.vaisseau.x < posBaseBx)) {
			checkpointCrossed(etat);
			isCheckpoint = false;
		}
		if (posYCheckpoint > Affichage.HAUT) {
			isCheckpoint = false;
		}
	}
	...
	private void setVieRecuperee(int i) {
		this.vieRecuperee = i;
	}
